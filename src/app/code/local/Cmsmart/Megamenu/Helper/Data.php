   ...
   public function toOptionArray()
	...
	public function _categotyFilter()
	{
		...
		$categoryFilterCollections = Mage::getModel('megamenu/megamenu')
            ...
            ;
//Mage::log($categoryFilterCollections->getSelect()->__toString());
        $categoryFilterCollections = $categoryFilterCollections->getData();

		$categoryFilterCollectionsAll = Mage::getModel('megamenu/megamenu')
            ->getCollection()
            ->addFieldToFilter('category_id', $category_id)
            ;
//        Mage::log($categoryFilterCollectionsAll->getSelect()->__toString());
        $categoryFilterCollectionsAll = $categoryFilterCollectionsAll->getData();
    ...
    public function getShowthumbnail($id){
        ...
        if(isset($thumbnail[0]) && isset($thumbnail[0]['active_thumbail'])) {
